...
bool GameOver::init()
{
	...
	label_score->setPosition(ccp(size.width / 2 - label_score->getContentSize().width / 2 + 20, size.height * 0.8 - label_score->getContentSize().height / 2 - 5));
	this->addChild(label_score);

	auto label_score_num = Label::create();
	label_score_num->setSystemFontSize(40);
	label_score_num->setAnchorPoint(ccp(0, 0.5));
	label_score_num->setPosition(ccp(label_score->getPositionX() + label_score->getContentSize().width / 2 + 20, label_score->getPositionY()));
	...
	label_best_score->setPosition(ccp(size.width / 2 - label_best_score->getContentSize().width / 2 + 20, label_score->getPositionY() - label_best_score->getContentSize().height - 5));
	this->addChild(label_best_score);

	auto label_best_score_num = Label::create();
	label_best_score_num->setSystemFontSize(40);
	label_best_score_num->setAnchorPoint(ccp(0, 0.5));
	label_best_score_num->setPosition(ccp(label_best_score->getPositionX() + label_best_score->getContentSize().width / 2 + 20, label_best_score->getPositionY()));
	...
	btn_label->setString(((String*)(dictionary->objectForKey("back_menu")))->getCString());
...
void GameOver::menu_play(Ref * ref)//返回主页面
{
